...
<head>
    ...
    <title>MedSync - Kelola Pasien (Resepsionis)</title>
...
<body class="bg-gray-100 flex h-screen overflow-hidden text-gray-800" x-data="{ sidebarOpen: false, isModalOpen: false, deleteUrl: '' }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-teal-800 text-white lg:translate-x-0 lg:static lg:inset-0 flex flex-col">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            ...
            <a href="{{ route('resepsionis.pasien') }}" class="block px-4 py-3 rounded-lg bg-teal-900 text-white font-semibold transition-colors">
    ...
    <main class="flex-1 flex flex-col relative h-screen overflow-y-auto">
        <header class="bg-white shadow-sm px-4 lg:px-8 py-4 flex justify-between items-center">
            <div class="flex items-center">
                ...
                <h2 class="text-xl font-semibold text-gray-700">Kelola Data Pasien</h2>
            </div>
        ...
        <div class="p-4 lg:p-8">
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                    <a href="{{ route('resepsionis.pasien.create') }}" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium transition-colors text-center shadow-sm">
                        + Tambah Pasien
                    </a>
                    <form action="{{ route('resepsionis.pasien') }}" method="GET" class="relative w-full sm:w-auto flex">
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama/alamat..." class="w-full sm:w-auto border border-gray-300 rounded-l-lg px-4 py-2 pl-10 focus:outline-none focus:ring-2 focus:ring-teal-500">
                        ...
                        <tbody class="text-sm">
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="p-4 flex justify-center space-x-2">
                                    <a href="{{ route('resepsionis.pasien.edit', $pasien->id_pasien) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded text-xs transition-colors shadow-sm">Ubah</a>
                                    
                                    <button type="button" @click="isModalOpen = true; deleteUrl = '{{ route('resepsionis.pasien.destroy', $pasien->id_pasien) }}'" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded text-xs transition-colors shadow-sm">
                                        Hapus
                                    </button>
                                </td>
                            </tr>
