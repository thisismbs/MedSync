        ...
        <div class="p-4 lg:p-8">
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                    <a href="{{ route('resepsionis.pasien.create') }}" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium transition-colors text-center">
                        + Tambah Pasien
                    </a>
                    <form action="{{ route('resepsionis.pasien') }}" method="GET" class="relative w-full sm:w-auto flex">
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama/HP..." class="w-full sm:w-auto border border-gray-300 rounded-l-lg px-4 py-2 pl-10 focus:outline-none focus:ring-2 focus:ring-teal-500">
                    ...
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        ...
                        <tbody class="text-sm">
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="p-4 truncate max-w-xs">{{ $pasien->alamat }}</td>
                                <td class="p-4">{{ $pasien->no_hp }}</td>
                                <td class="p-4">{{ \Carbon\Carbon::parse($pasien->tgl_lahir)->format('d-m-Y') }}</td>
                                <td class="p-4 flex justify-center space-x-2">
                                    <a href="{{ route('resepsionis.pasien.edit', $pasien->id_pasien) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs transition-colors">Ubah</a>
                                    <form action="{{ route('resepsionis.pasien.destroy', $pasien->id_pasien) }}" method="POST" onsubmit="return confirm('Yakin hapus data pasien ini?');">
                                        @csrf
                                        @method('DELETE')
                ...
                <div class="mt-4 flex justify-end">
                    <span class="text-sm text-gray-500">Menampilkan total {{ $pasiens->count() }} data pasien</span>
                </div>
            </div>
        </div>
    </main>
