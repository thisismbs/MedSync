...
<body class="bg-gray-100 flex h-screen overflow-hidden text-gray-800" x-data="{ sidebarOpen: false }">
    ...
    <main class="flex-1 flex flex-col relative h-screen overflow-y-auto">
        <header class="bg-white shadow-sm px-4 lg:px-8 py-4 flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <a href="{{ route('resepsionis.pasien') }}" class="text-gray-500 hover:text-teal-600 transition-colors">
        ...
        <div class="p-4 lg:p-8">
            <div class="bg-white rounded-xl shadow-md p-6 lg:p-8 max-w-3xl mx-auto">
                ...
                <form action="{{ isset($pasien) ? route('resepsionis.pasien.update', $pasien->id_pasien) : route('resepsionis.pasien.store') }}" method="POST" class="space-y-6">
                    @csrf
                    @if(isset($pasien))
                        @method('PUT')
                    @endif
    ...
    <input type="tel" name="no_hp" value="{{ $pasien->no_hp ?? '' }}" x-on:input="$el.value = $el.value.replace(/[^0-9]/g, '')" required placeholder="Contoh: 081234567890" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:outline-none">
                    ...
                    <div class="pt-6 mt-6 border-t flex justify-end space-x-3">
                        <a href="{{ route('resepsionis.pasien') }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium">
                            Batal
                        </a>
