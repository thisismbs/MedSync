...
<head>
    ...
    <title>MedSync - Data Pasien</title>
    ...
    <main class="flex-1 flex flex-col relative h-screen overflow-y-auto">
        <header class="bg-white shadow-sm px-4 lg:px-8 py-4 flex justify-between items-center">
            <div class="flex items-center">
                ...
                <h2 class="text-xl font-semibold text-gray-700">Manajemen Data Pasien</h2>
            </div>
        </header>
        ...
        <div class="p-4 lg:p-8">
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                    <a href="{{ Auth::user()->role == 'admin' ? route('admin.pasien.create') : route('resepsionis.pasien.create') }}" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium transition-colors text-center">
                        + Tambah Pasien
                    </a>
                    
                    <form action="{{ Auth::user()->role == 'admin' ? route('admin.pasien') : route('resepsionis.pasien') }}" method="GET" class="relative w-full sm:w-auto flex">
                        <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Cari nama pasien/alamat..." class="w-full border border-gray-300 rounded-l-lg px-4 py-2 pl-10 focus:outline-none focus:ring-2 focus:ring-teal-500">
                    ...
                    <table class="w-full text-left border-collapse whitespace-nowrap">
                        ...
                        <tbody class="text-sm">
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                <td class="p-4 text-gray-500 font-medium">{{ $pasiens->firstItem() + $index }}</td>
                                <td class="p-4 font-semibold text-gray-800">{{ $pasien->nama }}</td>
                                <td class="p-4">{{ \Carbon\Carbon::parse($pasien->tgl_lahir)->format('d-m-Y') }}</td>
                                <td class="p-4">{{ $pasien->no_hp }}</td>
                                <td class="p-4 text-gray-600 truncate max-w-xs" title="{{ $pasien->alamat }}">{{ \Illuminate\Support\Str::limit($pasien->alamat, 30) }}</td>
                                <td class="p-4 flex justify-center space-x-2">
                                    <a href="{{ Auth::user()->role == 'admin' ? route('admin.pasien.edit', $pasien->id_pasien) : route('resepsionis.pasien.edit', $pasien->id_pasien) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1.5 rounded text-xs transition-colors">Ubah</a>
                                    
                                    <button type="button" @click="isModalOpen = true; deleteUrl = '{{ Auth::user()->role == 'admin' ? route('admin.pasien.destroy', $pasien->id_pasien) : route('resepsionis.pasien.destroy', $pasien->id_pasien) }}'" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded text-xs transition-colors">
                                        Hapus
                                    </button>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="6" class="p-4 text-center text-gray-500">Belum ada data pasien terdaftar.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div x-show="isModalOpen" 
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">Hapus Data Pasien</h3>
                        <div class="mt-2">
                            <p class="text-sm text-gray-500">Apakah Anda yakin ingin menghapus data pasien ini? Peringatan: Pasien yang memiliki riwayat rekam medis tidak dapat dihapus sebelum riwayatnya dibersihkan.</p>
                        </div>
                    </div>
                </div>
