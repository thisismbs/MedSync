...
<head>
    ...
    <title>MedSync - Kelola Pasien (Admin)</title>
...
<body class="bg-gray-100 flex h-screen overflow-hidden text-gray-800" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-teal-800 text-white lg:translate-x-0 lg:static lg:inset-0 flex flex-col">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            ...
            <a href="{{ route('admin.pasien') }}" class="block px-4 py-3 rounded-lg bg-teal-900 text-white font-semibold transition-colors">
        ...
        <div class="p-4 lg:p-8">
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center mb-6 space-y-4 sm:space-y-0">
                    <a href="{{ route('admin.pasien.create') }}" class="w-full sm:w-auto bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium transition-colors text-center">
                        + Tambah Pasien
                    </a>
                    <form action="{{ route('admin.pasien') }}" method="GET" class="relative w-full sm:w-auto flex">
                        ...
                        <tbody class="text-sm">
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td class="p-4 flex justify-center space-x-2">
                                    <a href="{{ route('admin.pasien.edit', $pasien->id_pasien) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs transition-colors">Ubah</a>
                                    <form action="{{ route('admin.pasien.destroy', $pasien->id_pasien) }}" method="POST" onsubmit="return confirm('Yakin hapus data pasien ini?');">
                                        @csrf
                                        @method('DELETE')
